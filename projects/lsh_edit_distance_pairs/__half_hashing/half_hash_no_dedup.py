...
# read in docs one by one, and hash the first and second halves of each doc such that the key -> value pairs in the map are doc_half -> doc_num. for the time being i'm not worried about numbering the halves -- this would avoid comparison of hypothetical false positives the first half of one doc is identical to the second half of another doc. this is specifically designed for finding pairs of docs whose max word level edit distance is 1, and would not work for a max word-level edit distance > 1
...
# pairwise combinations are taken from each deduplicated list of documents that were hashed to the same bucket, where the first document in any pair is always the one whose index is lower
	...
	if len(doc_list) > 1:
		for candidate_pair in itertools.combinations(sorted(set(doc_list)), 2):
			...
			if docs[candidate_pair[0]] == docs[candidate_pair[1]]:
				...
				# print str(candidate_pair[0]) + ',' + str(candidate_pair[1])
