...
# return a hash function whose seed is specified by n and which returns values in the range specified by r
def hash_function(n, r):
	random.seed(n)
	mask = random.randrange(r)
	def myhash(x):
		return hash(x) ^ mask
	return myhash


# return a list of all k-shingles in string s
...
# return a list of all k-shingles in string s
def get_word_shingles(s, k):

	s = s.split()
	shingles = list()
	for i in range(0, len(s) - k + 1):
		shingles.append(" ".join(s[i:i + k]))
	return shingles
